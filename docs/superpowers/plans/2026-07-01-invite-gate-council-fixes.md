...
- [ ] **Step 1: Add operator check, collapse into one handler**
...
- [ ] **Step 2: Add tests for the authorization check**
...
- [ ] **Step 1: Gate non-invite callbacks by status**
...
- [ ] **Step 2: Add test for blocked-user callback rejection**
...
- [ ] **Step 1: Move `InviteGate` to wrap the whole layout**
...
- [ ] **Step 2: Verify no regressions in existing layout tests**
...
- [ ] **Step 1: Fix session-check redirect logic**
...
- [ ] **Step 2: Wrap `submit()` in try/catch/finally**
...
- [ ] **Step 1: Read `ExportModal.tsx` for the existing dialog pattern**
...
- [ ] **Step 2: Add dialog semantics + focus trap to `EmailModal`**
...
- [ ] **Step 5: Exclude `blocked` status from the email modal**
...
- [ ] **Step 1: Add a shared email validator/normalizer to `src/utils/validators.py`**
...
- [ ] **Step 2: Use it from `src/api/auth.py` and `src/telegram/webhook.py`**
...
- [ ] **Step 3: Drop the redundant client-side JS regex**
...
- [ ] **Step 1: Inline `_telegram_identity`**
...
- [ ] **Step 2: Skip the upsert when status is already `approved` and identity is unchanged**
...
- [ ] **Step 1: Add a comment in `middleware.py`**
...
- [ ] **Step 2: Log a warning when `OPERATOR_CHAT_ID` is unset**
