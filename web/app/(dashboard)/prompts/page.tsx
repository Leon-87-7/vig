...
import type { Template, TemplateFormState } from '@/lib/hooks/useTemplateList';

function CreateForm({ onCreated }: { onCreated: (values: TemplateFormState) => Promise<void> }) {
  const [values, setValues] = useState<TemplateFormState>({ name: '', description: '', extra_instructions: '' });
...
function UserTemplateRow({
  ...
  template: Template;
  onUpdate: (name: string, values: Partial<TemplateFormState>) => Promise<void>;
  onDelete: (name: string) => Promise<void>;
}) {
  const [editing, setEditing] = useState(false);
  const [editValues, setEditValues] = useState({ description: template.description, extra_instructions: template.extra_instructions });
  ...
  const handleDelete = async () => {
    ...
    try { await onDelete(template.name); } catch (err) { setDeleteError(err instanceof Error ? err.message : 'Delete failed'); }
  };
...
export default function PromptsPage() {
  const { templates, loading, fetchError, createTemplate, deleteTemplate, updateTemplate } = useTemplateList();
        ...
        <>
          ...
          <section>
            ...
            {userTemplates.length === 0 ? (
              ...
              <ul className="space-y-2">
                {userTemplates.map((t) => (
                  <UserTemplateRow key={t.name} template={t} onUpdate={updateTemplate} onDelete={deleteTemplate} />
                ))}
              </ul>
            )}
          </section>
